...
(async () => {
  ...
  const translations = [
    ...
    "ນີ້ແມ່ນແບບທໍາອິ{green_pipe.webp}ບທໍາອິ\nດຂອງເນື້ອສຽງທີ່ມີຮູບພາບ {gold_moon.png}",  // Lao
    "នេះគឺជាគំរូនៃ{green_pipe.webp}ບທໍາອິ\nអត្ថបទដែលមានរូបភាព {gold_moon.png}"                 // Khmer
  ];
  ...
  translations.forEach((text, index) => {
    ...
    textContainer.x = 200;
    textContainer.y = 20 + FONT_SIZE*3*index; // 50 pixels spacing between containers
  });
})();
